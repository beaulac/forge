...
describe TaxRate do
  ...
  it "should have unique country+province" do
    ...
    tr3 = TaxRate.new(:title => "Test3", :rate => 1.0, :country_id => countries(:us), :province_id => provinces(:ny).id)
    tr3.valid?.should_not == nil
    tr3.save

    tr4 = TaxRate.new(:title => "Test3", :rate => 1.0, :country_id => countries(:us), :province_id => provinces(:ny).id)
