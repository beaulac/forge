module ForgeHelper
  ...
  def asset_editor_html(asset, options = {})
    if asset.is_image?
      style = "";
      style += "float: left; margin: 3px 5px 3px 0px;" if options[:wrap] == "left"
      style += "float: right; margin: 3px 0px 3px 5px;" if options[:wrap] == "right"
      style += "width: #{options[:width]}px;" if options[:width]
      style += "height: #{options[:height]}px;" if options[:height]
      return image_tag(asset.attachment.url(:medium), :style => style).html_safe
    else
      return link_to(options[:title], asset.attachment.url)
    end
  end
